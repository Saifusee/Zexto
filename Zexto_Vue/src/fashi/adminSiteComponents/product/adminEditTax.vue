          ...
          <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <form enctype="multipart/form-data" @submit.prevent>
                    ...
                    <hr>
                    <button type="submit" class="form-control btn btn-secondary" @click="submitTax">Edit Tax</button>
                </form>
            </div>
          </div>
...
export default {
    ...
    created()
    {
        axios.get(`taxes/${this.id}/edit`)
        .then(response => 
        {
            this.methodTaxId(response.data.id);
            this.methodTaxName(response.data.tax_name);
            this.methodTaxDescription(response.data.tax_description);
            this.methodTaxPercentage(response.data.tax_percentage);
        })
        .catch(error => error)
    ...
    methods:
    {
        ...
        submitTax()
        {
            ...
            axios.post(`taxes/${this.computeTaxId}`, formdata)
            .then(response => 
            {
                this.creationErrors = {};
                this.methodTaxId("");
